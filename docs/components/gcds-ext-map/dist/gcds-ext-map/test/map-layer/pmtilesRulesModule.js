const sheet = new protomapsL.Sheet(`
<html>
  <body>
    <svg id="star" width="32px" height="32px" xmlns="http://www.w3.org/2000/svg">
      <image href="data:image/png;base64," width="16" height="16" />
    </svg>
  </body>
</html>
`);
const pmtilesRules = new Map();
const pmtilesRulesReady = sheet.load().then(() => {
  pmtilesRules.set('http://localhost:3333/test/map-layer/spearfish.pmtiles?theme=dark', {
    theme: { theme: 'dark' }
  });
  pmtilesRules.set(
    'http://localhost:3333/test/data/tiles/osmtile/{beans}/{foo}/{bar}.mvt?theme=light',
    { theme: { theme: 'light' } }
  );
  pmtilesRules.set(
    'http://localhost:3333/test/data/tiles/osmtile/{z}/{x}/{y}.mvt?theme=light',
    { theme: { theme: 'light' } }
  );
  return pmtilesRules;
});

export { pmtilesRules, pmtilesRulesReady };
