...
class LocationInput {
    ...
    constructor(name, position, axis, units, min, max, rel, layer) {
        ...
        if (!units && axis && !['i', 'j'].includes(axis)) {
            this.units = ContextMenu.Util.axisToCS(axis).toLowerCase();
        }
        else {
            this.units = units; // cs
        }
    ...
    validateInput() {
        ...
        if (this.units) {
            let axisCS = ContextMenu.Util.axisToCS(this.axis);
        ...
        if (this.units === 'pcrs' || this.units === 'gcrs') {
            ...
            for (let j = tileRange.min.y; j <= tileRange.max.y; j++) {
                for (let i = tileRange.min.x; i <= tileRange.max.x; i++) {
                    const coords = new ContextMenu.leafletSrcExports.Point(i, j);
