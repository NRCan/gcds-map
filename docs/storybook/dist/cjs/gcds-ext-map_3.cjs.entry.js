...
var locale = require('./locale-B3PmwiAq.js');
...
var LayerControl = ContextMenu.leafletSrcExports.Control.Layers.extend({
  options: {
    autoZIndex: false,
    sortLayers: true,
  ...
  initialize: function (overlays, options) {
    ContextMenu.leafletSrcExports.setOptions(this, options);
  ...
  onRemove: function (map) {
    ContextMenu.leafletSrcExports.DomEvent.off(
      this._container.getElementsByTagName('a')[0],
      'keydown',
      this._focusFirstLayer,
      this._container
  ...
  removeLayer: function (layer) {
    ContextMenu.leafletSrcExports.Control.Layers.prototype.removeLayer.call(this, layer);
    if (this._layers.length === 0) {
      this._container.setAttribute('hidden', '');
    }
  },
  ...
  _update: function () {
    ...
    ContextMenu.leafletSrcExports.DomUtil.empty(this._baseLayersList);
    ContextMenu.leafletSrcExports.DomUtil.empty(this._overlaysList);
  ...
  _addItem: function (obj) {
    ...
    obj.input.layerId = ContextMenu.leafletSrcExports.stamp(obj.layer);
  ...
  collapse: function (e) {
    ...
    ContextMenu.leafletSrcExports.DomUtil.removeClass(this._container, 'leaflet-control-layers-expanded');
...
var ReloadButton = ContextMenu.leafletSrcExports.Control.extend({
  options: {
    position: 'topleft'
  },
  ...
  onAdd: function (map) {
    ...
    let container = ContextMenu.leafletSrcExports.DomUtil.create('div', 'mapml-reload-button leaflet-bar');

    let link = ContextMenu.leafletSrcExports.DomUtil.create('button', 'mapml-reload-button', container);
    ...
    ContextMenu.leafletSrcExports.DomEvent.disableClickPropagation(link);
    ContextMenu.leafletSrcExports.DomEvent.on(link, 'click', ContextMenu.leafletSrcExports.DomEvent.stop);
    ContextMenu.leafletSrcExports.DomEvent.on(link, 'click', this._goReload, this);
  ...
  _updateDisabled: function () {
    setTimeout(() => {
      ContextMenu.leafletSrcExports.DomUtil.removeClass(this._reloadButton, 'leaflet-disabled');
      ...
      if (
        ...
        ContextMenu.leafletSrcExports.DomUtil.addClass(this._reloadButton, 'leaflet-disabled');
...
var ScaleBar = ContextMenu.leafletSrcExports.Control.Scale.extend({
  options: {
    maxWidth: 100,
    updateWhenIdle: true,
    position: 'bottomleft'
  ...
  onAdd: function (map) {
    ...
    this._container = ContextMenu.leafletSrcExports.DomUtil.create('div', 'mapml-control-scale');
    let scaleControl = ContextMenu.leafletSrcExports.Control.Scale.prototype.onAdd.call(this, map);
 ...
 */
const LocationMarker = ContextMenu.leafletSrcExports.Marker.extend({
  initialize(latlng, options) {
    ContextMenu.leafletSrcExports.Util.setOptions(this, options);
    this._latlng = latlng;
    this.createIcon();
  },
  ...
  createIcon() {
    ...
    this._locationIcon = new ContextMenu.leafletSrcExports.DivIcon({
  ...
  setStyle(style) {
    ContextMenu.leafletSrcExports.Util.setOptions(this, style);
    this.createIcon();
  }
});
...
const LocateControl = ContextMenu.leafletSrcExports.Control.extend({
  options: {
  ...
  onAdd(map) {
    ...
    this._map = map;
    this._layer = this.options.layer || new ContextMenu.leafletSrcExports.LayerGroup();
  ...
  async _activateCompass() {
    ...
    ContextMenu.leafletSrcExports.DomEvent.on(window, eventName, this._onDeviceOrientation, this);
  },
  ...
  _deactivateCompass() {
    ...
    this._compassHeading = null;
    ContextMenu.leafletSrcExports.DomEvent.off(window, this._compassEventName, this._onDeviceOrientation, this);
  ...
  _drawMarker() {
    ...
    if (this.options.drawCircle) {
      ...
      if (this._circle) {
        ...
        this._circle = new ContextMenu.leafletSrcExports.Circle(latlng, options).addTo(this._layer);
      }
    }
  ...
  _bindPopup(latlng, accuracy) {
    ...
    let popupText;
    if (typeof t === "string") {
      popupText = ContextMenu.leafletSrcExports.Util.template(t, data);
...
var GeolocationButton = ContextMenu.leafletSrcExports.Control.extend({
  options: {
  ...
  onAdd: function (map) {
    ...
    ContextMenu.leafletSrcExports.Control.CustomLocate = LocateControl.extend({
      start: function () {
    ...
    this.locateControl = new ContextMenu.leafletSrcExports.Control.CustomLocate({
      showPopup: false,
      strings: {
...
var FullscreenButton = ContextMenu.leafletSrcExports.Control.extend({
  options: {
    position: 'topleft',
    title: {
  ...
  onAdd: function (map) {
    ...
    var container = ContextMenu.leafletSrcExports.DomUtil.create(
      'div',
      'leaflet-control-fullscreen leaflet-bar leaflet-control'
    );

    this.link = ContextMenu.leafletSrcExports.DomUtil.create(
      'a',
    ...
    ContextMenu.leafletSrcExports.DomEvent.on(this.link, 'click', this._click, this);
  ...
  _click: function (e) {
    ContextMenu.leafletSrcExports.DomEvent.stopPropagation(e);
    ContextMenu.leafletSrcExports.DomEvent.preventDefault(e);
...
ContextMenu.leafletSrcExports.Map.include({
  ...
  toggleFullscreen: function (options) {
    ...
    var mapEl = ContextMenu.Util.getClosest(
      this.getContainer(),
      'gcds-ext-map'
    );
  ...
  _enablePseudoFullscreen: function (container) {
    ContextMenu.leafletSrcExports.DomUtil.addClass(container, 'leaflet-pseudo-fullscreen');
    this._setFullscreen(true);
    this.fire('fullscreenchange');
  },

  _disablePseudoFullscreen: function (container) {
    ContextMenu.leafletSrcExports.DomUtil.removeClass(container, 'leaflet-pseudo-fullscreen');
  ...
  _setFullscreen: function (fullscreen) {
    ...
    var container = ContextMenu.Util.getClosest(
      this.getContainer(),
      'gcds-ext-map'
    );
    if (fullscreen) {
      ContextMenu.leafletSrcExports.DomUtil.addClass(container, 'mapml-fullscreen-on');
    } else {
      ContextMenu.leafletSrcExports.DomUtil.removeClass(container, 'mapml-fullscreen-on');
    }
    this.invalidateSize();
  },

  _onFullscreenChange: function (e) {
    var fullscreenElement = ContextMenu.Util.getClosest(this.getContainer(), ':fullscreen'),
      mapEl = ContextMenu.Util.getClosest(this.getContainer(), 'gcds-ext-map');
...
ContextMenu.leafletSrcExports.Map.mergeOptions({
  fullscreenControl: false
});

ContextMenu.leafletSrcExports.Map.addInitHook(function () {
  ...
  if (fullscreenchange) {
    var onFullscreenChange = ContextMenu.leafletSrcExports.Util.bind(this._onFullscreenChange, this);

    this.whenReady(function () {
      ContextMenu.leafletSrcExports.DomEvent.on(document, fullscreenchange, onFullscreenChange);
    });

    this.on('unload', function () {
      ContextMenu.leafletSrcExports.DomEvent.off(document, fullscreenchange, onFullscreenChange);
    });
  }
});
...
var SearchButton = ContextMenu.leafletSrcExports.Control.extend({
  options: {
    position: 'topleft'
  },
  _getLocale: function (map) {
  ...
  onAdd: function (map) {
    let locale = this._getLocale(map);
    this._locale = locale;
    let container = ContextMenu.leafletSrcExports.DomUtil.create('div', 'mapml-search-control leaflet-bar');

    let button = ContextMenu.leafletSrcExports.DomUtil.create(
      'button',
    ...
    ContextMenu.leafletSrcExports.DomEvent.disableClickPropagation(button);
    ContextMenu.leafletSrcExports.DomEvent.on(button, 'click', ContextMenu.leafletSrcExports.DomEvent.stop);
    ContextMenu.leafletSrcExports.DomEvent.on(button, 'click', this._openPanel, this);
    ContextMenu.leafletSrcExports.DomEvent.on(
      button,
      'keydown',
      function (e) {
        if (e.key === 'Enter') {
          ContextMenu.leafletSrcExports.DomEvent.stop(e);
          this._openPanel();
        }
      },
      this
    ...
    let panel = ContextMenu.leafletSrcExports.DomUtil.create('div', 'mapml-search-panel');
    panel.setAttribute('hidden', '');
    map.getContainer().appendChild(panel);
    ContextMenu.leafletSrcExports.DomEvent.disableClickPropagation(panel);
    ContextMenu.leafletSrcExports.DomEvent.on(panel, 'mouseenter', function () {
      map.scrollWheelZoom.disable();
    });
    ContextMenu.leafletSrcExports.DomEvent.on(panel, 'mouseleave', function () {
      map.scrollWheelZoom.enable();
    });
    ContextMenu.leafletSrcExports.DomEvent.on(panel, 'wheel', ContextMenu.leafletSrcExports.DomEvent.stopPropagation);

    let input = ContextMenu.leafletSrcExports.DomUtil.create('input', 'mapml-search-input', panel);
    ...
    ContextMenu.leafletSrcExports.DomEvent.disableClickPropagation(input);
    ContextMenu.leafletSrcExports.DomEvent.on(
      input,
      'keydown',
      function (e) {
    ...
    let results = ContextMenu.leafletSrcExports.DomUtil.create('div', 'mapml-search-results', panel);
    this._results = results;

    let closeBtn = ContextMenu.leafletSrcExports.DomUtil.create(
      'button',
    ...
    ContextMenu.leafletSrcExports.DomEvent.disableClickPropagation(closeBtn);
    ContextMenu.leafletSrcExports.DomEvent.on(closeBtn, 'click', ContextMenu.leafletSrcExports.DomEvent.stop);
    ContextMenu.leafletSrcExports.DomEvent.on(closeBtn, 'click', this._closePanel, this);
  ...
  onRemove: function () {
    ...
    ContextMenu.leafletSrcExports.DomEvent.off(this._button, 'click', this._openPanel, this);
    ContextMenu.leafletSrcExports.DomEvent.off(this._closeBtn, 'click', this._closePanel, this);
  },
  ...
  _navigateToFeature: function (feature) {
    ...
    if (bbox && bbox.length === 4) {
      ...
      map.fitBounds(ContextMenu.leafletSrcExports.latLngBounds([south, west], [north, east]));
    } else if (
      feature.geometry &&
...
var StaticButton = ContextMenu.leafletSrcExports.Control.extend({
  options: {
    position: 'topright'
  },
  _getLocale: function (map) {
  ...
  onAdd: function (map) {
    ...
    let container = ContextMenu.leafletSrcExports.DomUtil.create('div', 'mapml-static-control leaflet-bar');
    ...
    ContextMenu.leafletSrcExports.DomEvent.disableClickPropagation(button);
    ContextMenu.leafletSrcExports.DomEvent.on(button, 'click', ContextMenu.leafletSrcExports.DomEvent.stop);
    ContextMenu.leafletSrcExports.DomEvent.on(button, 'click', this._toggleStatic, this);
...
var DebugOverlay = ContextMenu.leafletSrcExports.Layer.extend({
  onAdd: function (map) {
    ...
    if (mapSize.x > 400 || mapSize.y > 300) {
      this._container = ContextMenu.leafletSrcExports.DomUtil.create('table', 'mapml-debug', map._container);
...
var DebugPanel = ContextMenu.leafletSrcExports.Layer.extend({
  initialize: function (options) {
    ContextMenu.leafletSrcExports.setOptions(this, options);
  },

  onAdd: function (map) {
    this._title = ContextMenu.leafletSrcExports.DomUtil.create(
      'caption',
      'mapml-debug-banner',
      this.options.pane
    );
    ...
    map.debug = {};
    map.debug._infoContainer = this._debugContainer = ContextMenu.leafletSrcExports.DomUtil.create(
      'tbody',
      'mapml-debug-panel',
      this.options.pane
    );
    ...
    map.debug._tileCoord = ContextMenu.leafletSrcExports.DomUtil.create(
      'tr',
      'mapml-debug-coordinates',
      infoContainer
    );
    map.debug._tileMatrixCoord = ContextMenu.leafletSrcExports.DomUtil.create(
      'tr',
      'mapml-debug-coordinates',
      infoContainer
    );
    map.debug._mapCoord = ContextMenu.leafletSrcExports.DomUtil.create(
      'tr',
      'mapml-debug-coordinates',
      infoContainer
    );
    map.debug._tcrsCoord = ContextMenu.leafletSrcExports.DomUtil.create(
      'tr',
      'mapml-debug-coordinates',
      infoContainer
    );
    map.debug._pcrsCoord = ContextMenu.leafletSrcExports.DomUtil.create(
      'tr',
      'mapml-debug-coordinates',
      infoContainer
    );
    map.debug._gcrsCoord = ContextMenu.leafletSrcExports.DomUtil.create(
      'tr',
      'mapml-debug-coordinates',
      infoContainer
    );
  ...
  onRemove: function () {
    ContextMenu.leafletSrcExports.DomUtil.remove(this._title);
    if (this._debugContainer) {
      ContextMenu.leafletSrcExports.DomUtil.remove(this._debugContainer);
...
var DebugGrid = ContextMenu.leafletSrcExports.GridLayer.extend({
  initialize: function (options) {
    ContextMenu.leafletSrcExports.setOptions(this, options);
    ContextMenu.leafletSrcExports.GridLayer.prototype.initialize.call(this, this._map);
  },

  createTile: function (coords) {
    let tile = ContextMenu.leafletSrcExports.DomUtil.create('div', 'mapml-debug-tile');
...
var DebugVectors = ContextMenu.leafletSrcExports.LayerGroup.extend({
  initialize: function (options) {
    ContextMenu.leafletSrcExports.setOptions(this, options);
    ContextMenu.leafletSrcExports.LayerGroup.prototype.initialize.call(this, this._map, options);
  },
  onAdd: function (map) {
    ...
    let center = map.options.crs.transformation.transform(
      ContextMenu.leafletSrcExports.point(0, 0),
      map.options.crs.scale(0)
    );
    this._centerVector = ContextMenu.leafletSrcExports.circle(map.options.crs.pointToLatLng(center, 0), {
      radius: 250,
    ...
    setTimeout(() => {
        ...
        if (layers[i].layerBounds || layers[i].extentBounds) {
          ...
          if (layers[i].layerBounds) {
            boundsArray = [
              ...
              ContextMenu.leafletSrcExports.point(layers[i].layerBounds.max.x, layers[i].layerBounds.min.y),
              layers[i].layerBounds.max,
              ContextMenu.leafletSrcExports.point(layers[i].layerBounds.min.x, layers[i].layerBounds.max.y)
            ];
          } else {
            boundsArray = [
              layers[i].extentBounds.min,
              ContextMenu.leafletSrcExports.point(layers[i].extentBounds.max.x, layers[i].extentBounds.min.y),
              layers[i].extentBounds.max,
              ContextMenu.leafletSrcExports.point(layers[i].extentBounds.min.x, layers[i].extentBounds.max.y)
            ];
          }
      ...
      if (map.totalLayerBounds) {
        let totalBoundsArray = [
          ...
          ContextMenu.leafletSrcExports.point(map.totalLayerBounds.max.x, map.totalLayerBounds.min.y),
          map.totalLayerBounds.max,
          ContextMenu.leafletSrcExports.point(map.totalLayerBounds.min.x, map.totalLayerBounds.max.y)
        ];
...
var ProjectedExtent = ContextMenu.leafletSrcExports.Path.extend({
  getCenter: function (round) {
    let crs = this._map.options.crs;
    return crs.unproject(ContextMenu.leafletSrcExports.bounds(this._locations).getCenter());
  },
  ...
  _project: function () {
    ...
    for (let i = 0; i < this._locations.length; i++) {
      ...
      this._rings.push(ContextMenu.leafletSrcExports.point(pt0.x, pt0.y)._subtract(map.getPixelOrigin()));
    }
...
var Crosshair = ContextMenu.leafletSrcExports.Layer.extend({
  onAdd: function (map) {
    ...
    this._container = ContextMenu.leafletSrcExports.DomUtil.create('div', 'mapml-crosshair', map._container);
  ...
  onRemove: function (map) {
    ...
    ContextMenu.leafletSrcExports.DomEvent.off(map._container, 'keydown keyup mousedown', this._isMapFocused);
  },
  ...
  _addOrRemoveMapOutline: function (e) {
    ...
    if (this._map.isFocused && !this._outline) {
      this._outline = ContextMenu.leafletSrcExports.DomUtil.create('div', 'mapml-outline', mapContainer);
    } else if (!this._map.isFocused && this._outline) {
      ContextMenu.leafletSrcExports.DomUtil.remove(this._outline);
...
var FeatureIndexOverlay = ContextMenu.leafletSrcExports.Layer.extend({
  onAdd: function (map) {
    ...
    this._container = ContextMenu.leafletSrcExports.DomUtil.create(
      'div',
    ...
    this._output = ContextMenu.leafletSrcExports.DomUtil.create(
      'output',
      'mapml-feature-index',
      map._container
    );
    ...
    this._body = ContextMenu.leafletSrcExports.DomUtil.create(
      'span',
  ...
  _calculateReticleBounds: function () {
    ...
    let minPoint = ContextMenu.leafletSrcExports.point(center.x - w, center.y + h);
    let maxPoint = ContextMenu.leafletSrcExports.point(center.x + w, center.y - h);
    let b = ContextMenu.leafletSrcExports.bounds(minPoint, maxPoint);
    return ContextMenu.Util.pixelToPCRSBounds(
      b,
      this._map.getZoom(),
  ...
  _checkOverlap: function (e) {
    ...
    keys.forEach((i) => {
      ...
      let b = ContextMenu.leafletSrcExports.bounds();

      if (layers) {
        let keys = Object.keys(layers);
        keys.forEach((j) => {
          if (!b)
            b = ContextMenu.leafletSrcExports.bounds(
        ...
        b = ContextMenu.leafletSrcExports.bounds(layer._bounds.min, layer._bounds.max);
      }
...
const GcdsExtMap = class {
    ...
    get extent() {
        let map = this._map, pcrsBounds = ContextMenu.Util.pixelToPCRSBounds(map.getPixelBounds(), map.getZoom(), map.options.projection);
        let formattedExtent = ContextMenu.Util._convertAndFormatPCRS(pcrsBounds, map.options.crs, this.projection);
    ...
    _createMap() {
        if (!this._map) {
            this._map = ContextMenu.leafletSrcExports.map(this._container, {
                center: new ContextMenu.leafletSrcExports.LatLng(this.lat, this.lon),
                minZoom: 0,
    ...
    async _ensureControlsLoaded() {
        ...
        try {
            ...
            await Promise.resolve().then(function () { return require('./AttributionButton-D8mZf9fq.js'); });
            // Load ContextMenu handler to register init hooks
            await Promise.resolve().then(function () { return require('./index-KnvdY5iL.js'); }).then(function (n) { return n.ContextMenu$1; });
            await Promise.resolve().then(function () { return require('./index-KnvdY5iL.js'); }).then(function (n) { return n.AnnounceMovement$1; });
            await Promise.resolve().then(function () { return require('./index-KnvdY5iL.js'); }).then(function (n) { return n.keyboard; });
            // Load FeatureIndex handler to register init hooks for keyboard navigation
            await Promise.resolve().then(function () { return require('./index-KnvdY5iL.js'); }).then(function (n) { return n.FeatureIndex$1; });
    ...
    _createControls() {
        ...
        if (!this._zoomControl && totalSize + 93 <= mapSize) {
            totalSize += 93;
            this._zoomControl = ContextMenu.leafletSrcExports.control
                .zoom({
        ...
        this.el.addEventListener('keydown', (e) => {
            if (e.keyCode === 86 && e.ctrlKey) {
                navigator.clipboard.readText().then((layer) => {
                    ContextMenu.Util._pasteLayer(this.el, layer);
                });
            }
    ...
    _dropHandler(event) {
        ...
        ContextMenu.Util._pasteLayer(this.el, text);
    }
    ...
    zoomToExtent(west, south, east, north) {
        ...
        this._map.fitBounds(ContextMenu.leafletSrcExports.latLngBounds([+south, +west], [+north, +east]));
        this._updateMapCenter();
    }
    zoomTo(lat, lon, zoom) {
        ...
        const location = new ContextMenu.leafletSrcExports.LatLng(+lat, +lon);
    ...
    geojson2mapml(json, options = {}) {
        ...
        let geojsonLayer = ContextMenu.Util.geojson2mapml(json, options);
...
var MapLayer = ContextMenu.leafletSrcExports.LayerGroup.extend({
  options: {
    zIndex: 0,
    opacity: '1.0'
  },
  ...
  initialize: function (href, layerEl, options) {
    ...
    ContextMenu.leafletSrcExports.DomUtil.addClass(this._container, 'mapml-layer');
  ...
  _calculateBounds: function () {
    ...
    bnds =
      ...
      this._layerEl.shadowRoot.querySelector(
        ...
        ? ContextMenu.Util.getBoundsFromMeta(this._layerEl.shadowRoot)
        : this._layerEl.querySelector(':scope > map-meta[name=extent][content]')
        ? ContextMenu.Util.getBoundsFromMeta(this._layerEl)
        : undefined;
    zoomBounds =
      this._layerEl.src &&
      this._layerEl.shadowRoot.querySelector(
        ...
        ? ContextMenu.Util.getZoomBoundsFromMeta(this._layerEl.shadowRoot)
        : this._layerEl.querySelector(':scope > map-meta[name=zoom][content]')
        ? ContextMenu.Util.getZoomBoundsFromMeta(this._layerEl)
        : undefined;
      ...
      if (type === '_extentLayer' && mapExtents.length) {
        for (let i = 0; i < mapExtents.length; i++) {
          if (mapExtents[i]._extentLayer?.bounds) {
            ...
            if (!bnds) {
              bnds = ContextMenu.leafletSrcExports.bounds(
        ...
        this.eachLayer(function (layer) {
          ...
          if (layer instanceof ContextMenu.MapFeatureLayer && layer.layerBounds) {
            if (!bnds) {
          ...
          if (layer instanceof ContextMenu.MapFeatureLayer && layer.zoomBounds) {
            if (!zoomBounds) {
    ...
    if (bnds) {
      ...
      this.bounds = ContextMenu.leafletSrcExports.bounds(projectionBounds.min, projectionBounds.max);
    }
  ...
  onRemove: function (map) {
    ContextMenu.leafletSrcExports.LayerGroup.prototype.onRemove.call(this, map);
    ContextMenu.leafletSrcExports.DomUtil.remove(this._container);
  ...
  _initialize: function () {
    ...
    function parseLicenseAndLegend() {
      ...
      if (licenseLink) {
        ...
        var licenseUrl = ContextMenu.Util.sanitizeUrl(
      ...
      ContextMenu.leafletSrcExports.setOptions(layer, { attribution: attText });
      var legendLink = mapml.querySelector('map-link[rel=legend]');
      if (legendLink) {
        var safeLegendUrl = ContextMenu.Util.sanitizeUrl(
  ...
  _attachSkipButtons: function (e) {
    ...
    if (popup._container.querySelector('nav[class="mapml-focus-buttons"]')) {
      ContextMenu.leafletSrcExports.DomUtil.remove(
        popup._container.querySelector('nav[class="mapml-focus-buttons"]')
      );
      ContextMenu.leafletSrcExports.DomUtil.remove(popup._container.querySelector('hr'));
    }
    //add when popopen event happens instead
    let div = ContextMenu.leafletSrcExports.DomUtil.create('nav', 'mapml-focus-buttons');
    // creates |< button, focuses map
    let mapFocusButton = ContextMenu.leafletSrcExports.DomUtil.create('button', 'mapml-popup-button', div);
    ...
    let previousButton = ContextMenu.leafletSrcExports.DomUtil.create('button', 'mapml-popup-button', div);
    previousButton.type = 'button';
    previousButton.title = map.options.mapEl.locale.kbdPrevFeature;
    previousButton.innerHTML = "<span aria-hidden='true'>&#10094;</span>";
    ContextMenu.leafletSrcExports.DomEvent.on(previousButton, 'click', layer._previousFeature, popup);

    // static feature counter that 1/1
    let featureCount = ContextMenu.leafletSrcExports.DomUtil.create('p', 'mapml-feature-count', div),
    ...
    let nextButton = ContextMenu.leafletSrcExports.DomUtil.create('button', 'mapml-popup-button', div);
    nextButton.type = 'button';
    nextButton.title = map.options.mapEl.locale.kbdNextFeature;
    nextButton.innerHTML = "<span aria-hidden='true'>&#10095;</span>";
    ContextMenu.leafletSrcExports.DomEvent.on(nextButton, 'click', layer._nextFeature, popup);

    // creates >| button, focuses map controls
    let controlFocusButton = ContextMenu.leafletSrcExports.DomUtil.create(
      'button',
      'mapml-popup-button',
      div
    );
    ...
    let divider = ContextMenu.leafletSrcExports.DomUtil.create('hr', 'mapml-popup-divider');
    ...
    function focusFeature(focusEvent) {
      ...
      if (
        ...
        setTimeout(() => {
          ...
          group.focus();
          ContextMenu.leafletSrcExports.DomEvent.stop(focusEvent);
        }, 0);
      } else if (
    ...
    function focusMap(focusEvent) {
      ...
      if (
        ...
        ContextMenu.leafletSrcExports.DomEvent.stopPropagation(focusEvent);
        map.closePopup(popup);
        map._container.focus();
    ...
    function attachZoomLink(e) {
      ...
      let tL = featureEl.extent.topLeft.gcrs,
        ...
        center = ContextMenu.leafletSrcExports.latLngBounds(
          ContextMenu.leafletSrcExports.latLng(tL.horizontal, tL.vertical),
          ContextMenu.leafletSrcExports.latLng(bR.horizontal, bR.vertical)
        ).getCenter(true);
...
var createLayerControlHTML = async function () {
  var fieldset = ContextMenu.leafletSrcExports.DomUtil.create('fieldset', 'mapml-layer-item'),
    input = ContextMenu.leafletSrcExports.DomUtil.create('input'),
    layerItemName = ContextMenu.leafletSrcExports.DomUtil.create('span', 'mapml-layer-item-name'),
    settingsButtonNameIcon = ContextMenu.leafletSrcExports.DomUtil.create('span'),
    layerItemProperty = ContextMenu.leafletSrcExports.DomUtil.create(
      'div',
      'mapml-layer-item-properties',
      fieldset
    ),
    layerItemSettings = ContextMenu.leafletSrcExports.DomUtil.create(
      'div',
      'mapml-layer-item-settings',
      fieldset
    ),
    itemToggleLabel = ContextMenu.leafletSrcExports.DomUtil.create(
      'label',
      'mapml-layer-item-toggle',
      layerItemProperty
    ),
    layerItemControls = ContextMenu.leafletSrcExports.DomUtil.create(
      'div',
    ...
    opacity = ContextMenu.leafletSrcExports.DomUtil.create('input'),
    opacityControlSummary = ContextMenu.leafletSrcExports.DomUtil.create('summary'),
    svgSettingsControlIcon = ContextMenu.leafletSrcExports.SVG.create('svg'),
    settingsControlPath1 = ContextMenu.leafletSrcExports.SVG.create('path'),
    settingsControlPath2 = ContextMenu.leafletSrcExports.SVG.create('path'),
    extentsFieldset = ContextMenu.leafletSrcExports.DomUtil.create('fieldset', 'mapml-layer-grouped-extents'),
  ...
  let removeControlButton = ContextMenu.leafletSrcExports.DomUtil.create(
    'button',
  ...
  ContextMenu.leafletSrcExports.DomEvent.on(removeControlButton, 'click', ContextMenu.leafletSrcExports.DomEvent.stop);
  ContextMenu.leafletSrcExports.DomEvent.on(
    removeControlButton,
    'click',
    (e) => {
  ...
  let itemSettingControlButton = ContextMenu.leafletSrcExports.DomUtil.create(
    'button',
  ...
  layerItemName.id = 'mapml-layer-item-name-{' + ContextMenu.leafletSrcExports.stamp(layerItemName) + '}';
  opacityControlSummary.innerText = mapEl.locale.lcOpacity;
  opacityControlSummary.id =
    'mapml-layer-item-opacity-' + ContextMenu.leafletSrcExports.stamp(opacityControlSummary);
...
const GcdsMapLayer = class {
    ...
    get extent() {
        ...
        return this._layer
            ? Object.assign(ContextMenu.Util._convertAndFormatPCRS(this._layer.bounds, window.M[this.getProjection()], this.getProjection()), { zoom: this._layer.zoomBounds })
            : null;
    }
    ...
    getProjection() {
        ...
        if (mapml.querySelector('map-meta[name=projection][content]')) {
            projection =
                ContextMenu.Util._metaContentToObject(mapml
    ...
    _attachedToMap() {
        ...
        ContextMenu.leafletSrcExports.setOptions(this._layer, {
            zIndex: position,
            mapprojection: proj,
    ...
    _validateDisabled() {
        ...
        const countFeatureLayers = () => {
            ...
            let disabledCount = 0;
            this._layer.eachLayer((layer) => {
                if (layer instanceof ContextMenu.MapFeatureLayer) {
                    totalCount++;
                    if (!layer.isVisible())
                        disabledCount++;
                }
    ...
    getAlternateStyles(styleLinks) {
        if (styleLinks.length > 1) {
            ...
            for (let j = 0; j < styleLinks.length; j++) {
                ...
                ContextMenu.leafletSrcExports.DomUtil.addClass(stylesControl, 'mapml-layer-item-style mapml-control-layers');
            }
    ...
    zoomTo() {
        this.whenReady().then(() => {
            let map = this.getMapEl()?._map, extent = this.extent, tL = extent.topLeft.pcrs, bR = extent.bottomRight.pcrs, layerBounds = ContextMenu.leafletSrcExports.bounds(ContextMenu.leafletSrcExports.point(tL.horizontal, tL.vertical), ContextMenu.leafletSrcExports.point(bR.horizontal, bR.vertical)), center = map.options.crs.unproject(layerBounds.getCenter(true));
            let maxZoom = extent.zoom.maxZoom, minZoom = extent.zoom.minZoom;
            map.setView(center, ContextMenu.Util.getMaxZoom(layerBounds, map, minZoom, maxZoom), {
                animate: false
            });
        });
    }
    ...
    mapml2geojson(options = {}) {
        return ContextMenu.Util.mapml2geojson(this.el, options);
    }
    render() {
        return null;
    }
...
exports.gcds_ext_map = GcdsExtMap;
exports.map_caption = MapCaption;
exports.map_layer = GcdsMapLayer;
